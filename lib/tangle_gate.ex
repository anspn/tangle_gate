defmodule TangleGate do
  ...
  ## Local Operations (no network required)
      ...
      {:ok, payload} = TangleGate.notarize("Hello, IOTA!")

  ## Ledger Operations (require IOTA Rebased node)

      # Publish a DID on-chain
      {:ok, published} = TangleGate.publish_did(secret_key: "iotaprivkey1...")

      # Create a locked notarization on-chain
      {:ok, notarization} = TangleGate.create_notarization(
  ...
  alias TangleGate.Credential
  alias TangleGate.Identity
  alias TangleGate.Notarization
  alias TangleGate.Session
  ...
  ## Examples
      ...
      iex> {:ok, result} = TangleGate.generate_did()
      iex> String.starts_with?(result.did, "did:iota:0x")
      true

      iex> {:ok, result} = TangleGate.generate_did(network: :smr)
  ...
  ## Examples
      ...
      iex> TangleGate.valid_did?("did:iota:0x123")
      true

      iex> TangleGate.valid_did?("not-a-did")
      false
  """
  ...
  ## Examples
      ...
      iex> TangleGate.create_did_url("did:iota:0x123", "key-1")
  ...
  **Warning**: This operation is irreversible. Once deactivated, the DID
  ...
  ## Examples
      ...
      {:ok, result} = TangleGate.create_credential(issuer_doc, holder_did, "AccessCredential", claims, key, frag)
  ...
  ## Examples
      ...
      iex> {:ok, payload} = TangleGate.notarize("test")
      iex> is_binary(payload["data_hash"]) and String.length(payload["data_hash"]) == 64
      true
  """
  @spec notarize(binary(), String.t()) :: {:ok, map()} | {:error, term()}
  defdelegate notarize(data, tag \\ "tangle_gate"), to: Notarization.Server, as: :create_payload
  ...
  defdelegate enqueue_notarization(data, tag \\ "tangle_gate"),
  ...
  defdelegate nif_ready?(), to: TangleGate.NIF.Loader, as: :ready?

  @doc """
  Get NIF information.
  """
  @spec nif_info() :: map()
  defdelegate nif_info(), to: TangleGate.NIF.Loader, as: :info
end
