defmodule TangleGate.Application do
  @moduledoc """
  ...
  ├── TangleGate.NIF.Loader          # Ensures NIF is loaded before other services
  ├── TangleGate.Store.Repo           # MongoDB connection pool
  ├── TangleGate.Identity.Supervisor  # DID-related services (one_for_one)
  │   ├── TangleGate.Identity.Server  # GenServer for DID operations
  │   └── TangleGate.Identity.Cache   # ETS-backed DID document cache
  ├── TangleGate.Credential.Supervisor  # VC/VP services (one_for_one)
  │   ├── TangleGate.Credential.ChallengeCache  # ETS-backed challenge nonce store
  │   └── TangleGate.Credential.Server  # GenServer for VC/VP operations
  ├── TangleGate.Notarization.Supervisor  # Notarization services (one_for_one)
  │   ├── TangleGate.Notarization.Server  # GenServer for notarization ops
  │   └── TangleGate.Notarization.Queue   # Pending notarization queue
  ├── TangleGate.Session.Supervisor   # TTY session services (one_for_one)
  │   └── TangleGate.Session.Manager  # Session recording & notarization
  └── Bandit (HTTP server)             # Serves REST API + frontend
      └── TangleGate.Web.Router       # Plug router
  ```
  ...
  def start(_type, _args) do
    ...
    opts = [strategy: :rest_for_one, name: TangleGate.Supervisor]
  ...
  defp web_children do
    if Application.get_env(:tangle_gate, :start_web, true) do
      port = Application.get_env(:tangle_gate, :port, 4000)
      Logger.info("Starting web server on 0.0.0.0:#{port}")

      [{Bandit, plug: TangleGate.Web.Router, port: port, ip: {0, 0, 0, 0}}]
    else
      []
    end
  end
  ...
  defp ensure_mongo_indexes do
    if Application.get_env(:tangle_gate, :start_repo, true) do
      TangleGate.Store.NotarizationStore.ensure_indexes()
      TangleGate.Store.CredentialStore.ensure_indexes()
      TangleGate.Store.UserStore.ensure_indexes()
    end
  rescue
    e ->
      Logger.warning(
