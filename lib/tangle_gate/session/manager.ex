defmodule TangleGate.Session.Manager do
  @moduledoc """
  ...
  alias TangleGate.Store.NotarizationStore
  ...
  def handle_call({:start_session, did, user_id}, _from, state) do
    ...
    # Notify connected agent about the new session
    TangleGate.Agent.WS.notify_session_started(%{
      session_id: session_id,
      user_id: user_id,
      did: did,
      pid_hint: nil
  ...
  def handle_call({:terminate_session, session_id}, _from, state) do
    case :ets.lookup(@table, session_id) do
      [{^session_id, session}] when session.status == :active ->
        ...
        # Also request termination via agent WebSocket
        TangleGate.Agent.WS.request_termination(session_id)
  ...
  defp do_end_session(session, state) do
    ...
    hash = TangleGate.Notarization.Server.hash_data(document_json)

    # Create notarization payload
    notarization_result =
      case TangleGate.Notarization.Server.create_payload(document_json, "tty_session") do
  ...
  # Also notify connected agent that the session ended
  defp notify_session_ended(session_id) do
    TangleGate.Agent.WS.notify_session_ended(session_id)
  end
  ...
  defp maybe_publish_on_chain(hash, session) do
    ...
    if secret_key && secret_key != "" do
      ...
      TangleGate.Notarization.Server.create_on_chain(opts)
    else
  ...
  defp load_persisted_sessions do
    if not Application.get_env(:tangle_gate, :start_repo, true) do
  ...
  defp repo_started?, do: Application.get_env(:tangle_gate, :start_repo, true)
  ...
  defp sessions_dir do
    Application.get_env(:tangle_gate, :sessions_dir) ||
      Path.join(:code.priv_dir(:tangle_gate) |> to_string(), "sessions")
  end
