defmodule TangleGate.Credential.Server do
  @moduledoc """
  ...
  defp emit_telemetry(operation, start_time, metadata) do
    ...
    :telemetry.execute(
      [:tangle_gate, :credential, operation],
      %{duration: duration},
      metadata
    )
  end
  ...
  defp load_server_identity do
    ...
    mongo_identity =
      if Application.get_env(:tangle_gate, :start_repo, true) do
        case TangleGate.Store.CredentialStore.get_server_identity() do
    ...
    if mongo_identity do
      ...
      case TangleGate.Vault.Client.read_server_did() do
        {:ok, vault_identity} ->
          Logger.info("Server DID restored from Vault: #{vault_identity.did}")

          # Backfill MongoDB so next restart finds it there
          if Application.get_env(:tangle_gate, :start_repo, true) do
            TangleGate.Store.CredentialStore.store_server_identity(vault_identity)
          end
  ...
  defp do_provision_server_did(opts) do
    secret_key =
      Keyword.get_lazy(opts, :secret_key, fn ->
        Application.get_env(:tangle_gate, :secret_key)
      end)
    ...
    unless secret_key && secret_key != "" do
      ...
      publish_opts =
        ...
        |> maybe_put(
          :node_url,
          Keyword.get(opts, :node_url) || Application.get_env(:tangle_gate, :node_url)
        )
        |> maybe_put(
          :identity_pkg_id,
      ...
      case TangleGate.Identity.Server.publish_did(publish_opts) do
        {:ok, did_result} ->
          identity = %{
            did: did_result.did,
          ...
          # Persist to MongoDB
          if Application.get_env(:tangle_gate, :start_repo, true) do
            TangleGate.Store.CredentialStore.store_server_identity(identity)
          end

          # Persist to Vault (survives MongoDB wipes / container rebuilds)
          TangleGate.Vault.Client.write_server_did(identity)
