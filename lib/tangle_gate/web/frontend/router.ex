defmodule TangleGate.Web.Frontend.Router do
  @moduledoc """
  ...
  alias TangleGate.Web.Frontend.Templates
  ...
  # --- Root / Dashboard ---------------------------------------------------
  get "/" do
    nif_info = TangleGate.nif_info()
    cache_stats = TangleGate.Identity.Cache.stats()
    queue_stats = TangleGate.queue_stats()
  ...
  get "/portal" do
    html =
      Templates.render(:portal, %{
        login_required: login_required?(),
        ttyd_url: Application.get_env(:tangle_gate, :ttyd_url, "http://localhost:7681")
      })
  ...
  defp login_required? do
    Application.get_env(:tangle_gate, :login_required, true)
  end
end
