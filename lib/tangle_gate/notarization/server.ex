defmodule TangleGate.Notarization.Server do
  @moduledoc """
  ...
  def create_payload(data, tag \\ "tangle_gate") when is_binary(data) and is_binary(tag) do
  ...
  defp ledger_opt(opts, key) do
    Keyword.get_lazy(opts, key, fn ->
      case key do
        :node_url ->
          Application.get_env(:tangle_gate, :node_url, "https://api.testnet.iota.cafe")

        :notarize_pkg_id ->
          Application.get_env(:tangle_gate, :notarize_pkg_id, "")
      end
    end)
  end
  ...
  defp emit_telemetry(operation, start_time, metadata) do
    ...
    :telemetry.execute(
      [:tangle_gate, :notarization, operation],
      %{duration: duration},
      metadata
    )
  end
