defmodule IotaService.Vault.Client do
  @moduledoc """
  ...
  ## Configuration
      ...
      config :iota_service, IotaService.Vault.Client,
        ...
        secret_path: "secret/data/iota_service"
  ...
  ## Parameters
  - `key` — The key path under the KV mount (e.g., `"iota_service"`)
  ...
  Called during application startup. Fetches the `iota_service` secret
  and applies known keys (e.g., `secret_key`) to the `:iota_service` config.
  ...
  def load_secrets do
    ...
    unless Keyword.get(config, :enabled, false) do
      ...
      secret_key_path = Keyword.get(config, :secret_path, "iota_service")
  ...
  defp apply_secrets(data) do
    ...
    Enum.each(secret_mappings, fn {vault_key, config_key} ->
      case Map.get(data, vault_key) do
        ...
        value ->
          Application.put_env(:iota_service, config_key, value)
