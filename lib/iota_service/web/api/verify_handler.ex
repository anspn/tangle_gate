defmodule IotaService.Web.API.VerifyHandler do
  @moduledoc """
  ...
  post "/hash" do
    ...
    if is_nil(data) || data == "" do
      ...
      hash = IotaService.Notarization.Server.hash_data(data)
  ...
  # ---------------------------------------------------------------------------
  get "/:object_id" do
    case IotaService.Notarization.Server.read_on_chain(object_id) do
      {:ok, result} ->
      ...
      {:error, reason} ->
        Helpers.json(conn, 422, %{
          ...
          message: "Failed to read on-chain notarization: #{inspect(reason)}"
        })
    end
  end
